        ...
        slotSelection.innerHTML = `
            ...
            <div class="time-slots">
                ${timeSlots.map(time => {
                    ...
                    const nowPerth = moment.tz('Australia/Perth');
                    const isToday = date === nowPerth.format('YYYY-MM-DD');
                    const slotMinutes = timeToMinutes(time);
                    const nowMinutes = nowPerth.hours() * 60 + nowPerth.minutes();
                    const isInPast = isToday && slotMinutes <= nowMinutes;
                    if (isInPast) return '';
                    return `
